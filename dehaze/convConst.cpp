...
// convolve one column of I by a 2rx1 min filter
void convMinY( float *I, float *O, float *T, int h, int r ) {
  int y, y0, y1, yi, m=2*r+1;
  #define min1(a,b) a<b ? a : b;
  #define mink(y0,y1) { O[y]=I[y0]; \
    for( yi=y0+1; yi<=y1; yi++ ) { if(I[yi]<O[y]) O[y]=I[yi]; }}
  for( y=0; y<r; y++ ) { y1=y+r; if(y1>h-1) y1=h-1; mink(0,y1); }
  for( ; y<=h-m-r; y+=m ) {
    T[m-1] = I[y+r];
    for( yi=1; yi<m; yi++ ) T[m-1-yi] = min1( T[m-1-yi+1], I[y+r-yi] );
    for( yi=1; yi<m; yi++ ) T[m-1+yi] = min1( T[m-1+yi-1], I[y+r+yi] );
    for( yi=0; yi<m; yi++ ) O[y+yi] = min1( T[yi], T[yi+m-1] );
  }
  for( ; y<h-r; y++ ) { mink(y-r,y+r); }
  for( ; y<h; y++ ) { y0=y-r; if(y0<0) y0=0; mink(y0,h-1); }
  #undef mink
  #undef min1
}

// convolve I by a 2rx1 min filter
void convMin( float *I, float *O, int h, int w, int d, int r ) {
  if( r>w-1 ) r=w-1; if( r>h-1 ) r=h-1; int m=2*r+1;
  float *T=(float*) alMalloc(m*2*sizeof(float),16);
  for( int d0=0; d0<d; d0++ ) for( int x=0; x<w; x++ ) {
    float *Oc=O+d0*h*w+h*x, *Ic=I+d0*h*w+h*x;
    convMinY(Ic,Oc,T,h,r);
  }
  alFree(T);
}
...
void mexFunction( int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[] ) {
  ...
  } else if(!strcmp(type,"convMin")) {
    if( s>1 ) mexErrMsgTxt("convMin cannot sample");
    convMin( A, B, ns[0], ns[1], d, r );
  }  else {
